...
class Model {
    ...
    var tour = Dictionary<String, Any>()
    ...
    */
    ...
    func getTourData(userData: Dictionary<String, Any>) {
        let request = WebServiceRequest()
        request.httpMethod = "POST"
        request.urlBase = "https://ios2017.azurewebsites.net/api/tours"
        request.messageBody = userData as AnyObject?
        
        request.sendRequest(toUrlPath: "", dataKeyName: nil, completion: {
            (result: [AnyObject]) in
            self.tour = result[0] as! Dictionary<String, Any>
            self.delegate?.webServiceRequestDidChangeContent(model: self)
        })
    }
}
