...
class Model {
    let cdStack: CDStack
    let frc_friend: NSFetchedResultsController<Friend>
    ...
    init() {
        ...
        frc_friend = cdStack.frcForEntityNamed("Friend", withPredicateFormat: nil, predicateObject: nil, sortDescriptors: [NSSortDescriptor(key: "lastName", ascending: true)], andSectionNameKeyPath: nil)
    ...
    func delete(item: Friend) {
        context.delete(item)
    }
    
    func addNewFriend() -> Friend {
        guard let entity = NSEntityDescription.entity(forEntityName: "Friend", in: cdStack.managedObjectContext) else {
            fatalError("Can't create entity named Friend")
        }
        return Friend(entity: entity, insertInto: cdStack.managedObjectContext)
    }
    
    func addNewFriend(firstName: String, lastName: String) -> Friend {
        let newFriend = addNewFriend()
